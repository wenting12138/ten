package com.wen.recruit.service;
...
import com.wen.recruit.dao.RecruitDao;
...
public class RecruitService {
	 ...
	 *  推荐职位
	 */
	public ResultService<List<Recruit>> list(String state){
		List<Recruit> recruitList = recruitDao.findTop6ByStateOOrderByCreatetimeDesc(state);
		if (recruitList == null || recruitList.isEmpty()) {
			return new ResultService<>(false);
		}
		return new ResultService<>(true, recruitList.size(), recruitList);
	}

	/**
	 *  最新职位
	 */
	public ResultService<List<Recruit>> newList(String state){
		List<Recruit> recruitList = recruitDao.findTop6ByStateNotOrOrderByCreatetimeDesc(state);
		if (recruitList == null || recruitList.isEmpty()) {
			return new ResultService<>(false);
		}
		return new ResultService<>(true, recruitList.size(), recruitList);
